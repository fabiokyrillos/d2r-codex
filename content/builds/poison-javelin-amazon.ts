...
export const poisonJavelinAmazon: Build = {
  ...
  selfFoundNotes:
    "Cheap to start and genuinely gated at the end. Titan's Revenge, Razortail, Raven Frost and Atma's Scarab are all common Hell finds, Peace and Spirit are Countess runes, and both main skills are available before level 20. **Bramble is the wall**: Ral, Ohm, Sur and Eth is a serious rune investment, and it is the only source of poison skill damage in an armour slot, and the two Rainbow Facet rolls are the only others anywhere. A self-found version works and simply deals less damage — which on a damage-over-time build means fights that take longer rather than fights you lose.",
