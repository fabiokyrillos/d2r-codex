...
- **R-TREE-1 · Três árvores por classe, grade 3×6.** Cada tree renderiza 3 colunas × 6 fileiras (níveis 1/6/12/18/24/30) em **todas** as larguras a partir de 320 px, com as células vazias presentes no DOM **e nomeadas**. Corrigir o `role=grid` inválido de D7 exige as duas coisas: manter a célula vazia no DOM **e remover o `aria-hidden="true"` que hoje a tira da árvore de acessibilidade em qualquer largura, dando-lhe um nome acessível** — a chave `skills.emptyCell` ("No skill" / "Sem skill") já existe nos dois dicionários, sem uso, e é o nome previsto para isso. Só alargar a grade não corrige. O protótipo da Fase 0B.1 demonstrou o resultado: 18 gridcells, 3 por fileira, 8 células vazias nomeadas e nenhuma `aria-hidden`. A colapsagem para uma coluna é removida. Abaixo de 640 px, o trilho de nível é tratado conforme a saída do protótipo (hipótese inicial: suprimido, com o nível no cabeçalho de fileira e no `aria-label`). *Aceite:* teste de viewport em 320, 390, 640, 768, 1024, 1280 px afirma `gridTemplateColumns` com 3 faixas, 18 células por tree e sem overflow.
  > *Medido na Fase 4 (2026-09-11, build local `865bb3a`, não publicado; relatório em [`plans/phase-4-report.md`](plans/phase-4-report.md)):* 3 `role=grid` por classe, 18 `row`, 54 `gridcell` no DOM **e** na árvore de acessibilidade (baseline: 30 de 54), 24 vazias nomeadas `skills.emptyCell`, 3 faixas de coluna em 320/390/640/768/1024/1280 (baseline: 1/1/3/3/3/3), nos dois idiomas.
- **R-TREE-2 · Conectores sempre visíveis.** O SVG de dependências renderiza em todas as larguras, com traço sem escala e caminho do nó selecionado destacado. *Aceite:* SVG com `getClientRects().length > 0` em 320 px.
  > *Medido na Fase 4 (2026-09-11, build local `865bb3a`, não publicado; relatório em [`plans/phase-4-report.md`](plans/phase-4-report.md)):* peças `<span data-edge><svg><line vector-effect="non-scaling-stroke">` numa sobreposição `aria-hidden` colocada na própria subgrade; cada aresta toca origem e destino a ±1 px em 320/390/640/1280 e a 150 % de texto (192 verificações); o caminho de pré-requisito inteiro recebe `data-related` no hover, no foco e na seleção.
- **R-TREE-3 · Nó compacto responsivo.** O nó tem ícone, nome e contador de pontos no canto. **Números fixados pelo protótipo (variante F, aprovada em Q2):** em 280 px úteis, **nó de 88 px de largura, vão de 8 px** (3×88 + 2×8 = 280 exatos) e **fileira com piso de 74 px, elástica** — piso, nunca altura fixa, porque fileira fixa corta o nome com texto ampliado. Nome a **12 px** no estado normal. Limites que continuam valendo: nó ≥ 44 px nos dois eixos (F mede 88×74 a 100 % e 74,7×72 no tier de glifo); nome nunca omitido do `aria-label`; se o nome não for desenhado no nó, `title` e o painel carregam o nome completo. *Aceite:* medição em 320 px com estes números; teste de a11y confere `aria-label` = "Nome, nível N, Tree, estado, pontos".
  > *Medido na Fase 4 (2026-09-11, build local `865bb3a`, não publicado; relatório em [`plans/phase-4-report.md`](plans/phase-4-report.md)):* nó ≥ 88 × 74 a 320 px (fileira com piso de 74 px e elástica — 83 px na Summoning por "Raise Skeletal Mage" em três linhas), ≥ 44 × 44 em toda largura, nome a 12 px, grade de 484 px (493/495 na Necromancer).
...
- **R-TREE-8 · Abas por tree no celular (confirmado em Q2).** Abaixo de 640 px a seção mostra um controle com as três trees e renderiza **uma por vez**; a partir de 640 px, as três **empilhadas** (tablet mantém visão completa). As trees não ativas ficam fora da árvore de acessibilidade e do Tab (`hidden`, não `aria-hidden` isolado). **Legenda: uma só, compartilhada pela seção** — nunca uma por tree. Pode iniciar compacta ou recolhida, mas **continua expansível e explica todos os símbolos**. Baseline: 3.511 px de seção em 320 px. **Orçamento aprovado: ≤ 900 px por seção com uma tree em 320 px** (F mede 829,5 px na Cold e 877,7 px na Summoning, com a grade em 484 px). *Aceite:* troca de tree sem navegação e com foco preservado; nenhum elemento focável dentro de tree inativa; uma única legenda por seção, expansível e completa; sem JS, as três renderizam empilhadas; altura por tree dentro do orçamento.
  > *Medido na Fase 4 (2026-09-11, build local `865bb3a`, não publicado; relatório em [`plans/phase-4-report.md`](plans/phase-4-report.md)):* abas < 640 px como três links de fragmento enriquecidos a `tablist`, uma tree na página e as outras `hidden` (0 focáveis, fora da árvore de acessibilidade), troca sem navegação e com o foco na aba; ≥ 640 px empilhadas; uma legenda por seção. **Seção com uma tree a 320 px: Cold 778 (en) / 798 (pt), Summoning 787 / 807 — dentro dos 900.** A caixa da página de build definida pelo plano (§8.1: cabeçalho da seção + árvore) mede 903–992 px nas seis combinações — acima da estimativa do plano; **decisão do proprietário pendente (relatório §4, D3)**; nada foi apertado.
  > **Decisão do proprietário D3 (Fase 4, 2026-09-12, depois do UAT físico).** A **caixa da página de
  > build** a 320 px (topo da seção ao fundo das trees: cabeçalho com `skillsDescription` e total, abas
  > com pontos, tree com a sua linha de pontos, legenda, "Meu nível") tem contrato definitivo de
  > **≤ 1.050 px** — medida em 903–992 nas seis combinações — **preservando a descrição, os pontos nas
  > abas e o total da seção**. As páginas de classe continuam em ≤ 900 (medidas 778–807). O gate
  > `test:tree-browser` afirma os dois números.
...
- **R-TREE-13 · Teclado.** Todos os nós de cada tree são alcançáveis a partir do primeiro usando apenas setas (invariante atual, preservado); o movimento é espacialmente previsível: esquerda/direita percorrem a fileira, cima/baixo percorrem a coluna, pulando células vazias; quando não há nó na direção exata, o fallback é definido e documentado (próximo nó na mesma direção na fileira/coluna vizinha mais próxima, e continuação para a fileira vizinha na borda, para não romper a alcançabilidade); Home/End vão ao primeiro/último nó ocupado; Enter/Espaço abre; Esc fecha e devolve o foco; Tab sai da tree. *Aceite:* teste que, para as 24 árvores das 8 classes, todo nó é alcançável a partir do primeiro só com setas, Home e End; casos de previsibilidade em pelo menos três classes (Sorceress, Necromancer, Warlock), incluindo "Ice Blast → baixo = Glacial Spike" e o fallback documentado para "Ice Blast → direita".
  > *Medido na Fase 4 (2026-09-11, build local `865bb3a`, não publicado; relatório em [`plans/phase-4-report.md`](plans/phase-4-report.md)):* 24/24 árvores alcançáveis só com setas; linha e coluna estritas primeiro, continuação documentada depois (Ice Blast ↓ = Glacial Spike, → = Shiver Armor); 218 dos 480 movimentos verticais mudam em relação ao vizinho mais próximo; Home/End, Enter/Espaço, Esc (só com o foco na seção, ou com a sheet aberta), Tab sai; verificado num DOM real no desktop e no telefone.
- **R-TREE-14 · Leitor de tela.** `role=grid` mantido; cada célula anuncia nome, nível, tree, estado e pontos; conectores são decorativos (`aria-hidden`); o painel é `role=region` com nome; `aria-live=polite` só na seleção confirmada (Enter/Espaço/clique), nunca na mudança de foco; o sheet é `role=dialog`. *Aceite:* teste de composição de `aria-label` estendido aos estados; teste de que mover o foco não altera a região `aria-live`.
  > *Medido na Fase 4 (2026-09-11, build local `865bb3a`, não publicado; relatório em [`plans/phase-4-report.md`](plans/phase-4-report.md)):* `aria-label` "nome, nível N, tree, estado[, pontos]" composto do dicionário nos 30 nós de cada classe; região live escrita só na seleção confirmada e inalterada após setas e hover; painel `role=region` nomeado; sheet `role=dialog` modal; conectores `aria-hidden`.
- **R-TREE-15 · Zoom, reflow e texto ampliado.** Em 320 px CSS efetivos (equivalente a 400 % de zoom em 1280 px, critério WCAG 1.4.10) a grade continua 3×6 sem rolagem horizontal. Para **texto ampliado** (corpo escalado, que é diferente de zoom de página), vale a **estratégia B, aprovada em Q2**: preservar grade, posições, conectores, pontos e estados, e **substituir o nome dentro do nó por ícone/glifo quando ele não couber de forma legível**. Regras invariantes: **nunca partir palavras em fragmentos de uma ou duas letras**; `aria-label` e `title` mantêm o nome completo **sempre**, inclusive no modo glifo; **foco ou seleção mostra imediatamente o nome completo**; o painel/sheet contém todos os detalhes; a árvore nunca vira lista. **A troca para o modo glifo deve ser determinada por capacidade real de layout, não por um breakpoint arbitrário sem teste** — o limiar de 13em do protótipo é uma medição, não um contrato. A alternativa de fazer a fileira crescer indefinidamente com o nome a 12 px ("estratégia A") está **rejeitada**: medida a 200 % ela parte 3 nomes onde a variante E a 11 px partia 2. *Aceite:* teste de viewport em 320 px; a 200 % de texto, **zero palavras partidas e zero fragmentos de ≤ 2 letras**; `aria-label` e `title` com o nome completo em ambos os tiers; foco revela o nome.
  > *Medido na Fase 4 (2026-09-11, build local `865bb3a`, não publicado; relatório em [`plans/phase-4-report.md`](plans/phase-4-report.md)):* a 200 % em 320 e 390 px: grade 3×6, 0 palavras partidas, 0 fragmentos, 0 nomes cortados, seção sem rolagem horizontal, `title` e `aria-label` com o nome; modo glifo decidido por medição (sonda com `data-measuring`, `ResizeObserver`, converge) com piso CSS a 13,5em que reproduz o mesmo estado antes da ilha montar; a 100 % nenhuma das oito classes entra em glifo. O transbordo de 232 px do documento a 320/200 % é do header e é igual ao baseline.
- **R-TREE-16 · Sem JavaScript.** A grade 3×6, os conectores e os nós são HTML/SVG estático com links; apenas seleção, painel, estados dependentes de preferência e abas dependem de JS. *Aceite:* teste de HTML sem scripts encontra a grade e 30 links por classe.
  > *Medido na Fase 4 (2026-09-11, build local `865bb3a`, não publicado; relatório em [`plans/phase-4-report.md`](plans/phase-4-report.md)):* sem scripts a 320 px: `(scripting: enabled)` falso, ilha não montada, três trees empilhadas, 3 faixas, conectores, 30 links (clicar navega para a página da skill), abas como links de fragmento sem `role`, nenhum controle de preferência; nenhum `<noscript>`, nenhum script inline.
- **R-TREE-17 · Desempenho (resultado, não mecanismo).** A página de classe não pode ficar perceptivelmente mais lenta: o HTML por página de classe permanece dentro do teto proposto pelo spike da Fase 0B (saída declarada de R-TREE-19: número, baseline de 312 KB decodificados, método de medição e justificativa), sem imagens raster que impeçam recolorir por CSS e sem aumento do JS de cliente além do necessário para estados. Sprite SVG, `<symbol>/<use>`, teto por classe e formato de arquivo são hipóteses do spike. *Aceite:* medição do HTML por página de classe, pelo método declarado no spike, ≤ teto aprovado; ícones tematizáveis por CSS (cor do elemento e estados) em teste de contraste.
  > *Medido na Fase 4 (2026-09-11, build local `865bb3a`, não publicado; relatório em [`plans/phase-4-report.md`](plans/phase-4-report.md)):* 16 documentos de classe entre 181.931 e **247.950 bytes** (pt-br/sorceress; baseline 348.313, acima do alerta de 348.160), todos ≤ 368.640 e nenhum em alerta; JS de cliente da página de classe +20.638 bytes decodificados (a ilha substitui a camada interativa); nenhum elemento React no payload da ilha; CLS 0 numa chegada por `#skills`; a seção mede o mesmo antes e depois da hidratação.
...
| *↳ medido após a Fase 4 (2026-09-11, `865bb3a`, build local, não publicado)* | 3.511 (Cold en) / 3.761 (Summoning pt) | **778 / 798 (Cold en/pt), 787 / 807 (Summoning en/pt) — ≤ 900**; a caixa da build (plano §8.1) mede 903–992 px, decisão pendente (relatório §4) | `test:tree-browser` C2 |
| *↳ **decisão do proprietário D3 (2026-09-12, depois do UAT físico)*** | — | **Classe: ≤ 900 px** (mantido). **Build: ≤ 1.050 px** (contrato definitivo; medido 903–992), com descrição, pontos nas abas e total preservados | `test:tree-browser` C2 |
...
| *↳ medido após a Fase 4 (2026-09-11)* | 24/24 | **24/24 com linha/coluna estritas e a continuação documentada** (564 verificações; Ice Blast ↓ Glacial Spike, → Shiver Armor) | `test:nav`, `test:tree-browser` C5 |
...
| *↳ medido após a Fase 4 (2026-09-11, `865bb3a`)* | 348.313 bytes (pt-br/sorceress, antes de qualquer código da fase) | **247.950 bytes** no pior documento; os 16 entre 181.931 e 247.950, nenhum em alerta (348.160), teto 368.640 | `test:class-html-size` |
...
>
> **Fase 4 implementada localmente e parada no portão de UAT (2026-09-11, `865bb3a`).** Relatório em
> [`plans/phase-4-report.md`](plans/phase-4-report.md); evidência visual em
> [`plans/phase-4-visual/`](plans/phase-4-visual/README.md). Medido no build de produção local: grade
> 3×6 em toda largura, 54 de 54 `gridcell` na árvore de acessibilidade, conectores em 320 px, seção
> com uma tree a 320 px em 778–807 px nas páginas de classe (≤ 900), pior documento de classe em
> 247.950 bytes (era 348.313), 24/24 árvores por setas, CLS 0 numa chegada por `#skills`, altura
> igual antes e depois da hidratação. **Uma decisão fica com o proprietário (D3, relatório §4):** a
> caixa da página de build a 320 px, como o plano a definiu (cabeçalho da seção + árvore), mede
> 903–992 px contra 880–895 estimados; o gate a segura em 1.050 px provisórios, rotulados. **Nada
> foi enviado, mesclado ou implantado; a fase não está concluída** — o UAT físico decide.
>
> **UAT físico aprovado (2026-09-12).** O proprietário testou em telefone real, no mesmo Wi-Fi,
> Sorceress, Necromancer e Blizzard Sorceress, e aprovou os dez itens do portão: dez nós consecutivos,
> troca entre as três trees, sheet, rolagem sem toques acidentais, safe area, nomes no estado normal,
> grade 3×6 com texto ampliado, skill identificada no modo glifo, pt-BR e en-US, contadores e total
> da build, "Meu nível" em 18 com persistência e "Limpar". **D3** fixa a caixa da build em ≤ 1.050 px
> (R-TREE-8). A publicação e a verificação em produção estão no relatório §13; a fase só é marcada
> como concluída depois delas.
>
> **Fase 4 concluída (2026-09-12).** Push normal `436dc88..3d77ec6`; deployment `6414265076`
> (Production, `success`) do SHA exato `3d77ec6`; smoke público contra `https://d2r-codex.vercel.app`
> nos dois idiomas, desktop e mobile: `test:tree-browser` 2.759/2.759 (146 páginas com console
> limpo, sem JavaScript incluído), `test:viewport` 1.171/0, `test:a11y` 1.477 e
> `test:class-html-size` 22 sobre os documentos de produção (pior 248.575 bytes), 24 documentos sem
> `localhost`/rede local e com `canonical` absoluto. Relatório §13. **A Fase 5 não foi iniciada.**
...
| 2026-09-12 | **Fase 4 publicada e concluída.** `git push` normal de `436dc88..3d77ec6` (oito commits locais); `vercel[bot]` registrou o deployment `6414265076` (Production, `success`) para o SHA exato; produção a servir a árvore 3×6 55 s depois. Smoke público em pt-BR e en-US, desktop e mobile, com o driver do repositório apontado à produção: `test:tree-browser` 2.759/2.759 (árvores, abas, painel/sheet, `d2rc.level`, sem JavaScript, texto a 200 %, console limpo em 146 páginas), `test:viewport` 1.171/0, `test:a11y` 1.477 e `test:class-html-size` 22 sobre os documentos de produção (pior 248.575 bytes), nenhum `localhost` nem endereço de rede local, `canonical` absoluto. Uma calibração de gate saiu do smoke (C2b bloqueia `*/_next/static/*.js`, o caminho `immutable/` da Vercel). Fase 5 não iniciada. | §14, relatório §13 |
| 2026-09-12 | **UAT físico da Fase 4 aprovado; decisão D3.** Telefone real, mesmo Wi-Fi, Sorceress, Necromancer e Blizzard Sorceress: os dez itens do portão aprovados (toque em dez nós consecutivos, troca de trees, sheet, rolagem, safe area, nomes, 3×6 com texto ampliado, modo glifo, os dois idiomas, contadores e total, "Meu nível" 18 com persistência e "Limpar"). **D3:** a caixa da página de build a 320 px passa a ter contrato definitivo de ≤ 1.050 px (medida 903–992), preservando descrição, pontos nas abas e total; classe continua ≤ 900. O gate deixa de ser provisório. | R-TREE-8, §12.2, §14 |
| 2026-09-11 | **Fase 4 implementada localmente, parada no portão de UAT físico (`865bb3a`, não publicada).** Uma ilha por seção renderiza três `role=grid` 3×6 de dados compactos (o pior documento de classe cai de 348.313 para 247.950 bytes); células vazias no DOM e na árvore de acessibilidade; conectores em subgrade `aria-hidden` com o caminho de pré-requisito aceso; abas < 640 px com uma tree na página, empilhadas ≥ 640, três empilhadas sem JS por `(scripting: enabled)`; quatro estados por forma e texto e "Meu nível" (`d2rc.level`, D1); sheet modal < 1024 e painel ancorado; teclado espacial com a continuação documentada (24/24); modo glifo por medição com piso a 13,5em; a altura da seção é a mesma antes e depois da hidratação (esqueleto do controle de nível). Medido: classe 778–807 px a 320 (≤ 900); a caixa da build 903–992 — **D3 pendente**. 27 mutations aplicadas e revertidas; gates completos verdes. R-TREE-1…17 anotados com o medido. | R-TREE-1…17, §12.2, §14 |
