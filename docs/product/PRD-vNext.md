...
| D7 | Árvore vira lista de uma coluna e esconde os conectores abaixo de 640 px; as células vazias saem do DOM, o que deixa o `role=grid` com fileiras de larguras diferentes (inválido) | `skill-tree-interactive.tsx` (`hidden sm:block`) |
...
- **R-TREE-0 · Protótipo mobile aprovado antes de qualquer código de árvore.** Um HTML estático em `docs/` (sem código de produção) com as árvores Cold da Sorceress e Summoning do Necromancer (nomes mais longos: "Raise Skeletal Mage", "Corpse Explosion", "Skeleton Mastery"), em EN e PT, deve responder:
...
- **R-TREE-3 · Nó compacto responsivo.** O nó tem ícone, nome e contador de pontos no canto. Largura, vão e altura da fileira abaixo de 640 px são **saídas do protótipo**, com estes limites: três colunas cabem em 280 px úteis; nó ≥ 44 px nos dois eixos; nome nunca omitido do `aria-label`; se truncar visualmente, `title` e o painel carregam o nome completo. *Aceite:* medição em 320 px com os números aprovados; teste de a11y confere `aria-label` = "Nome, nível N, Tree, estado, pontos".
...
- **R-TREE-8 · Abas por tree no celular (hipótese até o protótipo).** Ponto de partida: abaixo de 640 px a seção mostra um controle com as três trees e renderiza uma por vez; acima, as três empilhadas como hoje (tablet mantém visão completa). As trees não ativas ficam fora da árvore de acessibilidade e do Tab (`hidden`, não `aria-hidden` isolado). Baseline: 3.511 px de seção em 320 px; meta: redução medida no protótipo e fixada no plano da Fase 4. *Aceite:* troca de tree sem navegação e com foco preservado; nenhum elemento focável dentro de tree inativa; sem JS, as três renderizam empilhadas; altura por tree ≤ orçamento aprovado no protótipo.
...
- **Dependências:** Q2 aprovada (Fase 0B).
...
- **6c Skill e busca:** P2.2 dano 1–30 e anterior/próximo; P2.6 tolerância a erro e busca única. Aceite: ≥ 7 pontos de dano; "blizard" encontra Blizzard.
