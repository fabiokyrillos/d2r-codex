...
import { GearPickView, gearSlotLabels, RichText } from "@/components/game";
 ...
 */
export function GearProgression({ gearSets }: { gearSets: GearSet[] }) {
    ...
    <div>
      ...
      >
        {ordered.map((set) => (
          ...
          >
            {progressionTiers[set.tier].label}
          </a>
        ))}
      </nav>
      ...
      <div className="space-y-10">
        {ordered.map((set, index) => {
          const meta = progressionTiers[set.tier];
          return (
            <section
              key={set.tier}
              id={`gear-${set.tier}`}
            ...
            >
              <header className="border-b border-border pb-4">
                <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                  ...
                  {set.levelRange && (
                    <Badge tone="outline">
                      Levels {set.levelRange[0]}–{set.levelRange[1]}
                    </Badge>
                  )}
                </div>
              ...
              {set.charms && set.charms.length > 0 && (
                <SupplementaryBlock title="Charms & inventory" picks={set.charms} />
              )}

              {set.weaponSwap && set.weaponSwap.length > 0 && (
                <SupplementaryBlock title="Weapon swap" picks={set.weaponSwap} />
              )}
